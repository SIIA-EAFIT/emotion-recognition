...
class Dataset:
    ...
    def convert_csv_to_images(self):
        ...
        for chunk in self.__gen_chunks(reader):
            for row in chunk:
                ...
                img = np.fromstring(
                    row[1], dtype=np.uint8, sep=' ').reshape(48, 48)
                imageio.imwrite(folder + str(indexs[int(row[0])]) + ".jpg",
                                img)
                indexs[int(row[0])] += 1
    ...
    def generate_hdf5_file_from_images(self):
        ...
        hdf5_file.create_dataset("train_labels", (len(train_addrs), ),
                                 np.uint8)
    ...
    def get_next_batch(self):
        ...
        end = min([(self.current_batch + 1) * self.batch_size,
                   self.data_length])  # index of the last image in this batch
